...
func (ts *closeTestSystem) createClient(index int) error {
	cli, err := NewClient(rdpnet.JoinHostPort("localhost", strconv.Itoa(ts.port)), ts.params)
		...
		default:
			...
			if err != nil {
				...
				if numEcho == ts.numClients*ts.numMsgs {
					...
					// XXX
					// 关闭服务器，那么服务器就不能再接收客户端连接，不能再提供服务了。
					// 而关闭服务器现有的所有连接，则服务器后面还可以继续提供服务。
...
func (ts *closeTestSystem) buildClient(clientID int) {
	...
	case ts.mode == doClientClose:
		...
		t.Logf("Closed client %d...", clientID)
...
// 下面两个测试带上-race后偶尔无法通过，但这是测试本身的问题。

...
func (ts *testSystem) clientTryRead(size int, expectedData []byte) {
	...
	for {
		...
		case SHORT:
			print("size: ", size)
			// 不知道为什么serverTryRead就把Fatalf注释掉，而这里则Fatalf。
...
func (ts *testSystem) testServerWithVariableLengthMsg(timeout int) {
	...
	rdpnet.SetMsgLengtheningPercent(100)
	...
	rdpnet.SetMsgLengtheningPercent(0)

	// Last, verify that server doesn't read short messages
	ts.t.Logf("Testing the server with a short message")
	rdpnet.SetMsgShorteningPercent(100)

	defer rdpnet.SetMsgShorteningPercent(0)
...
func (ts *testSystem) testServerWithCorruptedMsg(timeout int) {
	...
	rdpnet.SetMsgCorrupted(true)
	defer rdpnet.SetMsgCorrupted(false)
...
// 其实下面这两个测试我感觉没什么实际意义，
// 因为这里的测试是在端主机的包装过的UDP网络库rdpnet中做手脚，
// 仍然是在应用层人为扩展或截断要发送的数据，
// 这样由于不是在网络传输过程中导致的数据丢失或增加，这其实就绕过了UDP的校验和检查，
// 导致我们的应用层协议会受到扩展过或截断过的数据包，
// 如果是因为网络传输导致的数据丢失或增加，我们的应用层协议根本不必去检查，
// UDP的校验和的校验就不过关，直接丢弃，然后不ack，等待对端超时重传，
// 直到本端收到一个正确无误的数据包为止。

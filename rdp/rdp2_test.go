...
func newWindowTestSystem(t *testing.T, mode windowTestMode, numClients, numMsgs int, params *Params) *windowTestSystem {
	...
	hostport := rdpnet.JoinHostPort("localhost", strconv.Itoa(port))
...
func (ts *windowTestSystem) runMaxCapacityTest() {
	...
	// XXX 等到大概[1, 5]发完超时后，[6, 10]开始发送，恢复服务端，这样就会ack[6, 10]，但可能[1, 5]时间到，重传，导致误ack了[1, 5]，导致测试失败。
...
func TestOutOfOrderMsg1(t *testing.T) {
	rdpnet.SetDelayMessagePercent(50)
	defer rdpnet.SetDelayMessagePercent(0)
